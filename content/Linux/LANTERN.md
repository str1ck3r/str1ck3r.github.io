...
# Recon
### nmap
 Начинаем со сканирования открытых портом, используя `nmap`.
```bash
...
Расширение wapalyzer установило, что сайт написан на .Net-е. Используется KestrelHttpServer сервер и какой-то blazor. Код страницы выглядит следующим образом:
![[Pasted image 20260516100841.png]]
Я узнал, что [blazor](https://dotnet.microsoft.com/en-us/apps/aspnet/web-apps/blazor) фреймворк .Net или C#, который обрабатывает клиентскую и серверную часть приложения. Еще я заметил, что приложение работает на websocket-ах...  
...
Я решил поискать эксплойты в skipper proxy. Нашел много упоминаний SSRF уязвимости.
...
Я попробовал вставить мой ip в заголовок x-skipper-proxy. Получил вот такой меседж и подтвержение, что уязвимость присутствует:
...
```
Очень похож на 3000 порт, но есть отличия. 
...
#### Enumeration
Видим такую вот страницу. Имеем возможность добавить сотрудника. Я попробовал небольшие пэйлоды в эти поля, но ничего интересного не заметил... Также тут почему-то не отображается поле "Additional internal information"...
...
### SQL Injection
Вводя корректный id все работает нормально. Я попробовал добавить одну кавычку и я увидел возможность... Затем попровал это:
![[Pasted image 20260514112620.png]]

Также вот так могу:
![[Pasted image 20260514112905.png]]

Нужно вытащить как можно больше полезной информации. Я ввел следющие пейлоды:
...
* ' UNION SELECT 1,sql,2 FROM sqlite_master WHERE type!='meta' AND sql NOT NULL AND name ='Employees';-- - -> ( столбцы страницы )
...
```
Получяем пароль от админики. Я думал, что это пароль от ssh (типо там была приписка #22), но это был пароль от админки на 3000 порту.

### Different route
> Также этот пароль можно было достать другим способом. При входе на страницу порта 5000, загружается много разных dll-ок. Там есть одна интересная. Называется она InternalLantern.dll. Надо было ее скачать ( curl 'http://lantern.htb/_framework/InternaLantern.dll' -H "X-Skipper-Proxy: http://127.0.0.1:5000" -O IternaLantern.dll ) и используя такой инструмент как ilspycmd декомпилировать ее ( ilspycmd InternaLantern.dll -o ./decompile/ ).  Внутри этого кода можно было найти создание тех пользователей, которых мы видели при первом входе на 5000 порт. Там и можно было найти пароль от админки... 
...
### Arbitrary file read
Тут я увидел страницу, которую мне не получилось найти `/PrivacyAndPolicy`. Она просто загружает какой-то pdf файл. Мы можем передать 2 параметра `lang` и `file_ext`. И тут я попробовал такой пэйлоад для получения доступа к недоступным файлам ( path traversal ). Можем в параметр lang передать `.`, а в file_ext будет остальное, например `./../../../../../etc/passwd`.
...
Я хочу посмотреть как выглядит этот код. Скачиваем FileUpload.dll, перейдя в брайзере по 
```
...
```
Тут видно, что нет никакой санитизации файла. Код только заменяет обратный слэш. Можно попробовать в имя файла добавить "../" для выбора директории для сохранения.  
...
Теперь нужно серелиазовать. Получаем это:
```
...
Теперь отпускаем режим приема в burp-е и файл должен загрузиться в папку /tmp
...
Я декомпелировал наш dll эксплойт.
```C#
...
Переименуем Component1.razor на Component.razor... Я опять собрал это все. DLL-ка была успешно загружена. Теперь нам нужно написать основную часть...
...
Выбираем наш модуль и получаем reverse shell connection...
...
### Procmon
Недолго мыслив, я решил посмотреть, что мы можем запускать как `root`. Это программа [procmon](https://github.com/microsoft/ProcMon-for-Linux). По сути она служит для мониторинга и отслеживания системных вызовов там и все такое...
```
sudo -l
...
Мы видим разные системные вызовы. Нас интересуют `write` события, поэтому запускаем с опцией `-e write` для этого. Также из вывода команды `ps aux` мы узнали, что root открывал automation.sh при помощи `nano`. Поэтому будет брать только pid с этим процессом `-p $(pidof nano)`.
...
Затем сразу отправляем этот файл с захваченой системы:
```bash
...
### Getting root passwd
Этот столбец просто так не виден, тк это бинарные данные... Надо выводить в hex формате:
![[Pasted image 20260515084754.png]]
В этом столбце нам нужно как-то выводить байты `buf` ( буфер ). Я заметил, что его длинна зависит от `resultcode`. Поэтому будем скипать строки где resultcode = 0. Тут нужно написать скрипт для декодирования нужных строк:
...
```
Функция проходит по строкам. Если функция записи возвращает сообщение о том, что были записаны какие-либо байты, она получает это количество байтов из аргументов и выводит их. Там есть несколько символов \r для сброса курсора в начало строки, и я заменю их на символ новой строки, чтобы это было видно.

В начале я получал вот это...
![[Pasted image 20260515092154.png]]

Но я вообще не понял этот момент... Я запускал этот код несколько раз, и в один момент я получил это. Хотя файл был одним и тем же... Непонятно короче... 
![[Pasted image 20260515092249.png]]
Мы видим, что пароль передается для запуска /backup.sh. Еще символы выводяться по 2 раза... Учтя это, получаем пароль `Q3Eddtdw3pMB`. 
